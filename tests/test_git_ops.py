...
class TestConfigureCredentials:
    ...
    def test_configure_credentials_macos(self, mocker, tmp_path):
        """Test credential configuration on macOS."""
        ...
        assert mock_config_writer.set_value.call_count == 2
        mock_config_writer.set_value.assert_any_call("credential", "helper", "osxkeychain")
        mock_config_writer.set_value.assert_any_call("pull", "rebase", "true")

    def test_configure_credentials_windows(self, mocker, tmp_path):
        """Test credential configuration on Windows."""
        ...
        assert mock_config_writer.set_value.call_count == 2
        mock_config_writer.set_value.assert_any_call("credential", "helper", "wincred")
        mock_config_writer.set_value.assert_any_call("pull", "rebase", "true")

    def test_configure_credentials_linux(self, mocker, tmp_path):
        """Test credential configuration on Linux (no credential helper override)."""
        ...
        # Linux should only set pull.rebase, not credential helper
        mock_config_writer.set_value.assert_called_once_with("pull", "rebase", "true")
...
class TestInitRepository:
    ...
    def test_init_repository_handles_invalid_repo(self, mocker, tmp_path):
        ...
        mock_repo_class.clone_from.assert_called_once_with("https://github.com/user/repo.git", repo_path)
