...
class TestDiscoverOrcaSlicerPaths:
    ...
    def test_discover_windows_additional_locations(self, mocker, tmp_path):
        ...
        mocker.patch.dict(os.environ, {
            'APPDATA': str(tmp_path / "nonexistent"),
...
class TestConfigInitialization:
    ...
    def test_init_uses_default_profile(self, tmp_path):
        ...
        config_path.write_text(yaml.dump({
            ...
            'profiles': {
                'myprofile': {'branch_name': 'my-branch'}
            }
        }))
...
''')
    ...
    def test_load_yaml_with_nested_structures(self, tmp_path):
        ...
        assert config.data['profiles']['default']['paths']['Linux']['user_paths'] == ['/path/1', '/path/2']
...
class TestDefaultConfiguration:
    ...
    def test_default_config_uses_discovered_paths(self, tmp_path, mocker):
        ...
        assert any(str(orca_user) in str(p) for p in config.data['profiles']['default']['paths']['Linux']['user_paths'])
...
class TestProfileMerging:
    ...
    def test_profile_no_paths_for_platform(self, tmp_path, mocker):
        ...
        assert 'user_paths' not in config.active_profile or config.active_profile['user_paths'] is None
...
class TestConfigSaving:
    ...
    def test_save_preserves_structure(self, tmp_path):
        ...
        assert saved_data['profiles']['test']['paths']['Linux']['user_paths'] == ['/path/1', '/path/2']
...
class TestConfigProperties:
    ...
    def test_repository_url_property(self, tmp_path):
    ...
    def test_repository_url_property_empty(self, tmp_path):
        """Test repository_url property returns empty string when not set."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({}))
    ...
    def test_repository_name_property(self, tmp_path):
    ...
    def test_repository_name_property_default(self, tmp_path):
        """Test repository_name property returns default when not set."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({}))
        
        config = Config(config_path=config_path)
        
        assert config.repository_name == 'orca-profiles'
    ...
    def test_user_paths_property(self, tmp_path):
        """Test user_paths property returns list of Path objects."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({
            'user_paths': ['/path/1', '/path/2']
        }))
        ...
        assert all(isinstance(p, Path) for p in config.user_paths)
        assert config.user_paths[0] == Path('/path/1')
    
    def test_user_paths_property_empty(self, tmp_path):
        """Test user_paths property returns empty list when not set."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({}))
        
        config = Config(config_path=config_path)
        
        assert config.user_paths == []
    
    def test_system_paths_property(self, tmp_path):
        """Test system_paths property returns list of Path objects."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({
        ...
        assert len(config.system_paths) == 3
        assert all(isinstance(p, Path) for p in config.system_paths)
    
    def test_system_paths_property_empty(self, tmp_path):
        """Test system_paths property returns empty list when not set."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({}))
        
        config = Config(config_path=config_path)
        
        assert config.system_paths == []
    
    def test_sync_paths_property_combines_both(self, tmp_path):
        """Test sync_paths property combines user and system paths."""
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({
            'user_paths': ['/user/1', '/user/2'],
            'system_paths': ['/sys/1']
        }))
...
class TestEdgeCases:
    ...
    def test_config_with_unicode_characters(self, tmp_path):
        ...
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({
            'repository_name': 'プロファイル-配置文件-профиль'
        }))
    ...
    def test_config_with_special_path_characters(self, tmp_path):
        ...
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump({
            'user_paths': ['/path/with spaces/and-dashes', '/path/with_underscores']
        }))
    ...
    def test_config_with_very_long_values(self, tmp_path):
        ...
        long_value = 'x' * 10000
        config_path.write_text(yaml.dump({
            'commit_message_template': long_value
        }))
