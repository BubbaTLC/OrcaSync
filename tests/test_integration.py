...
class TestFileSyncOperations:
    ...
    def test_sync_directories(
        self, git_repo_path: Path, orcaslicer_profiles: Path, real_temp_dir: Path, num_sources
    ):
        """Test syncing files from single or multiple directories."""
        git_mgr = GitManager(git_repo_path, "", "main")
        git_mgr.init_repository()
        ...
        if num_sources == 1:
            ...
            # Create two profile directories for multiple sources test
        ...
        assert len(copied) > 0
        if num_sources == 1:
            # Check that files from OrcaSlicer profiles were copied
            assert (
                git_repo_path / "profiles" / "user" / "default" / "filament" / "PLA.json"
            ).exists()
        else:
...
class TestPushPullOperations:
    ...
    def test_pull_from_remote(
        self, git_repo_path: Path, remote_repo_path: Path, real_temp_dir: Path
    ):
...
class TestFullWorkflowIntegration:
    ...
    def test_full_push_workflow(
        self, git_repo_path: Path, orcaslicer_profiles: Path, remote_repo_path: Path
    ):
    ...
    def test_sync_workflow_with_conflicts(
        self, git_repo_path: Path, remote_repo_path: Path, real_temp_dir: Path
    ):
...
class TestErrorHandling:
    ...
    def test_restore_to_readonly_destination(self, git_repo_path: Path, real_temp_dir: Path):
        ...
        try:
            ...
            git_mgr.restore_files(sync_paths)
