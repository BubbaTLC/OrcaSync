...
class TestInitCommand:
    ...
    def test_init_first_time(self, runner, mocker, tmp_path):
        ...
        mocker.patch.object(Config, 'discover_orcaslicer_paths', return_value={
            "user": [tmp_path / "OrcaSlicer" / "user"],
            "system": []
        })
    ...
    def test_init_with_existing_config_cancel(self, runner, mocker, tmp_path):
        """Test init with existing config and user cancels."""
        config_path = tmp_path / "config.yaml"
        ...
        mocker.patch.object(Config, 'discover_orcaslicer_paths', return_value={"user": [], "system": []})
        ...
        assert not mock_cfg.save.called
    
    def test_init_with_existing_config_overwrite(self, runner, mocker, tmp_path):
        """Test init with existing config and user confirms overwrite."""
        config_path = tmp_path / "config.yaml"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text("existing: config")
        ...
        # Respond 'y' to overwrite, then provide repo config
    ...
    def test_init_with_discovered_paths(self, runner, mocker, tmp_path):
        ...
        set_calls = [call[0] for call in mock_cfg.set.call_args_list]
        assert any('user_paths' in call for call in set_calls)
        assert any('system_paths' in call for call in set_calls)
...
class TestPushCommand:
    ...
    def test_push_no_repo_url_cancel(self, runner, mock_config, mock_git_manager):
        """Test push with no repo URL and user cancels."""
        mock_config.repository_url = ""
        
        result = runner.invoke(push, input='n\n')
        
        assert result.exit_code == 0
        assert not mock_git_manager.init_repository.called
    
    def test_push_no_repo_url_continue(self, runner, mock_config, mock_git_manager):
        """Test push with no repo URL but user continues (local commit)."""
        mock_config.repository_url = ""
        mock_git_manager.sync_files.return_value = ["file1.json"]
        mock_git_manager.commit_changes.return_value = True
        
        result = runner.invoke(push, input='y\n')
        
        assert result.exit_code == 0
        assert mock_git_manager.commit_changes.called
        # Should not push when no remote URL
        assert not mock_git_manager.push.called
...
class TestStatusCommand:
    ...
    def test_status_with_repository(self, runner, mock_config, mock_git_manager, tmp_path):
        ...
        mocker_instance = pytest.importorskip('pytest_mock').mocker
