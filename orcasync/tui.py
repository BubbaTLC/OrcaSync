...
class StatusPanel(Static):
    ...
    def render_status(self) -> str:
        ...
        lines.append(
            f"[yellow]Repository:[/yellow] {cfg.repository_url or '[dim]Not configured[/dim]'}"
        )
        ...
        if repo_path.exists():
            try:
                ...
                status_text = (
                    "[yellow]Uncommitted changes[/yellow]" if dirty else "[green]Clean[/green]"
                )
                lines.append(f"  [yellow]Status:[/yellow] {status_text}")
...
class InitDialog(Screen):
    ...
    def handle_save(self) -> None:
        ...
        self.config.save()
        self.app_instance.get_compact_view().add_log(
            f"[green]✓ Configuration saved to {self.config.config_path}[/green]"
        )
...
class OrcaSyncApp(App[None]):
        ...
        with Container(id="main-container"):
            ...
            with Container(id="menu-container"):
                with Vertical(id="menu-grid"):
                    yield Static(
                        "[bold]Main Menu[/bold] (Use arrow keys or hotkeys)", id="menu-title"
                    )
                    with Horizontal():
    ...
    def get_status_panel(self) -> StatusPanel:
        ...
        return self.query_one(StatusPanel)  # type: ignore[no-any-return]

    def get_compact_view(self) -> CompactLogView:
        """Get the compact log view widget."""
        return self.query_one(CompactLogView)  # type: ignore[no-any-return]
    ...
    def initialize_repository(self) -> None:
        ...
        try:
            ...
            git_mgr = GitManager(
                self.repo_path, self.config.repository_url, self.config.branch_name
            )
            git_mgr.init_repository()
    ...
    def push_profiles(self) -> None:
        ...
        if not self.config.repository_url:
            log.add_log(
                "[yellow]⚠ No repository URL configured. Will commit locally only.[/yellow]"
            )
        ...
        try:
            ...
            try:
                if git_mgr.repo.active_branch.tracking_branch():
                    tracking = git_mgr.repo.active_branch.tracking_branch()
                    active = git_mgr.repo.active_branch
                    commits_ahead = list(
                        git_mgr.repo.iter_commits(f"{tracking.name}..{active.name}")
                    )
                    if commits_ahead:
                        git_mgr.push()
                        log.add_log(
                            f"[green]✓ Pushed {len(commits_ahead)} commit(s) to remote[/green]"
                        )
